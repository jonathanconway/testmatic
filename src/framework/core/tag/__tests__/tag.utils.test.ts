import { MOCK_TESTS, MOCK_TEST_WITH_MAPPED_TAGS } from "../../test";
import { MOCK_TAG, MOCK_TAGS, MOCK_TAG_LOGIN_SCREEN } from "../tag.mocks";
import {
  getStepsHavingTag,
  getTagsHavingTest,
  getTestsHavingTag,
...
describe("tag.utils", () => {
  describe("getTestsHavingTag", () => {
    it("filters tests by tag", () => {
      const result = getTestsHavingTag(MOCK_TESTS, MOCK_TAG);

      expect(result).toEqual([MOCK_TEST_WITH_MAPPED_TAGS]);
    });
  });
